...
import type { Category, InventoryItem, Product, Package, ProductModifierSlot, PackageItem, PackageItemModifierSlotOverride } from '../types/product-types'; // Import Package type

// --- Seed Data ---

const categories: Category[] = [
  { id: 'cat-alitas', name: 'Alitas', type: 'producto', imageUrl: 'https://i.pinimg.com/736x/82/2e/a7/822ea701f82eb9f0f9509b8458b08600.jpg' }, // Placeholder image
  ...
  // 'Paquete' category type might still be useful for UI grouping, but not essential for identifying packages
...
// Packages (Now in their own table)
const packages: Package[] = [
    { id: 'pkg-pareja', name: 'Combo Pareja', price: 270, imageUrl: 'https://picsum.photos/200/150?random=pareja' }
];
...
async function seedDatabase() {
  ...
  try {
    ...
    await db.run('DELETE FROM packages;'); // Clear packages table
    ...
    for (const product of products) { // Only insert regular products here
      await productStmt.run(
        ...
        product.inventory_consumed_per_unit ?? null
      );
    }
    ...
    console.log(`${products.length} products inserted.`);
     ...
     const packageStmt = await db.prepare('INSERT INTO packages (id, name, price, imageUrl) VALUES (?, ?, ?, ?)');
     for (const pkg of packages) {
        await packageStmt.run(pkg.id, pkg.name, pkg.price, pkg.imageUrl);
     }
